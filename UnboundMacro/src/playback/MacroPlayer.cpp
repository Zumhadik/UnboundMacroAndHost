...
constexpr std::uint64_t STARTUP_DELAY_MS = 3000;
constexpr std::uint64_t MOVE_UPDATE_MIN_MS = 8;
...
DWORD SleepChunk(std::uint64_t left_ms)
{
    if (left_ms > 32) {
        return 16;
    }
...
bool MacroPlayer::Start(HWND notify_hwnd, bool loop)
{
    ...
    startup_left_ms_ = static_cast<std::uint32_t>(STARTUP_DELAY_MS);
    playing_ = true;
...
bool MacroPlayer::IsStarting() const
{
    return playing_ && startup_left_ms_.load() > 0;
}

std::uint32_t MacroPlayer::StartupLeftMs() const
{
    return startup_left_ms_.load();
}

std::uint64_t MacroPlayer::ElapsedMs() const
{
    if (!playing_ || startup_left_ms_.load() > 0) {
        return 0;
    }
...
void MacroPlayer::RefreshScreenMetrics()
{
    screen_vx_ = GetSystemMetrics(SM_XVIRTUALSCREEN);
    screen_vy_ = GetSystemMetrics(SM_YVIRTUALSCREEN);
    screen_vw_ = GetSystemMetrics(SM_CXVIRTUALSCREEN);
    screen_vh_ = GetSystemMetrics(SM_CYVIRTUALSCREEN);
    if (screen_vw_ < 1) {
        screen_vw_ = 1;
    }
    if (screen_vh_ < 1) {
...
void MacroPlayer::EndInputProfile()
{
    ...
    SystemParametersInfoW(SPI_SETMOUSE, 0, saved_mouse_params_, 0);
    SystemParametersInfoW(
        SPI_SETMOUSESPEED,
        0,
    ...
    std::uint64_t target_ms)
{
    for (;;) {
        ...
        const DWORD chunk = SleepChunk(target_ms - elapsed);
        if (chunk > 0) {
            Sleep(chunk);
        }
    }
    ...
    int to_y)
{
    ...
    while (!stop_requested_) {
        ...
        if ((x != prev_x || y != prev_y) &&
            (last_move_ms == 0 ||
             elapsed - last_move_ms >= MOVE_UPDATE_MIN_MS)) {
            MoveTo(x, y);
            prev_x = x;
            prev_y = y;
            last_move_ms = elapsed;
        }
...
void MacroPlayer::ThreadMain()
{
    ...
    const std::uint64_t delay_start = Clock::NowMs();
    while (!stop_requested_) {
        const std::uint64_t elapsed = Clock::NowMs() - delay_start;
        if (elapsed >= STARTUP_DELAY_MS) {
            break;
        }
        const std::uint64_t left = STARTUP_DELAY_MS - elapsed;
        startup_left_ms_ = static_cast<std::uint32_t>(left);
        const DWORD chunk = SleepChunk(left);
        if (chunk > 0) {
            Sleep(chunk);
        }
    }
...
{
    const int vx = screen_vx_;
    const int vy = screen_vy_;
    const int vw = screen_vw_;
    const int vh = screen_vh_;
    if (vw <= 1 || vh <= 1) {
        return;
    }
